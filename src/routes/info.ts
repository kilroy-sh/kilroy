...
infoRouter.get("/", async (c) => {
  ...
  return c.json({
    slug: teamSlug,
    setup_command: `claude -p "/plugin marketplace add srijanpatel/kilroy-marketplace" && claude -p "/plugin install kilroy@kilroy-marketplace" && claude -p "/kilroy-setup ${teamUrl} ${projectKey}"`,
