...
export function createMcpServer(authUserId: string, authorType: "human" | "agent", baseUrl: string) {
  ...
  mcp.registerTool(
    ...
    {
      description:
        "Provision one or more single-use file upload slots. Each returned slot contains: `url` (the object's eventual URL — stable from this moment, even before the file is uploaded), `upload_curl` (a ready-to-run shell command that PUTs ONE file — fill in the `<file>`, `<mime>`, and `<filename>` placeholders), and `expires_at` (10-minute TTL on unconsumed slots). Pass the original filename via the `X-Kilroy-Filename` header so it shows up in attachment chips and in response metadata. To inspect an object without downloading it, send `HEAD <url>` — the response carries `Content-Type` (mime), `Content-Length` (size), `ETag` (sha256), `Content-Disposition` (filename), and `Last-Modified` (upload time). The slot UUID and the object UUID are the same value, so you can reference `url` in a Markdown post body (`![chart](URL)`) before running `upload_curl`. Objects are project-scoped and may be referenced from any post or comment in the project. Default count is 1; max 20.",
      inputSchema: {
        project: projectParam,
